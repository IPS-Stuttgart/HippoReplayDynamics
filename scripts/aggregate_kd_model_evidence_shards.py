...
def _exact_integer_text(
    ...
    *,
    ...
    if parsed < int(min_value):
        ...
        raise ValueError(
            f"CSV {key} must contain {qualifier} integer values: {path}"
        )
    return parsed


def _read_csv_with_exact_event_index(path: Path) -> pd.DataFrame:
    """Read a KD table while preserving exact decimal-form event identifiers."""
...
def _integer_metadata(
    ...
    *,
    ...
    if np.issubdtype(raw.dtype, np.integer):
        out_of_range = any(
            int(item) < intp_info.min or int(item) > intp_info.max
    ...
    if np.any(values < int(min_value)):
        qualifier = "nonnegative" if int(min_value) == 0 else "positive"
        raise ValueError(
            f"Momentum shard {key} must contain {qualifier} integer values: {path}"
        )
    return values
...
def _load_npz(path: Path) -> dict[str, object]:
    with np.load(path, allow_pickle=False) as shard:
        ...
        n_time = _integer_metadata(
            shard["n_time"],
            key="n_time",
            path=path,
            min_value=1,
        )
        n_spikes = _integer_metadata(
            shard["n_spikes"],
            key="n_spikes",
            path=path,
            min_value=0,
        ...
        if n_time.shape != event_ids.shape:
            raise ValueError(
                "Momentum shard n_time must match event_ids shape in "
                f"{path}: {n_time.shape} vs {event_ids.shape}"
            )
        if n_spikes.shape != event_ids.shape:
            raise ValueError(
                "Momentum shard n_spikes must match event_ids shape in "
                f"{path}: {n_spikes.shape} vs {event_ids.shape}"
            )
        return {
            "path": path,
            ...
            "kd_grid_preset": str(
                _np_scalar(shard["kd_grid_preset"], key="kd_grid_preset", path=path)
            ),
            "kd_time_bin_ms": float(
                _np_scalar(shard["kd_time_bin_ms"], key="kd_time_bin_ms", path=path)
            ),
            ...
            "kd_spike_rate_scale": float(
                _np_scalar(
                    shard["kd_spike_rate_scale"],
                    key="kd_spike_rate_scale",
                    path=path,
                )
            )
...
def _coerce_grid_index_array(shard: dict[str, object], key: str) -> np.ndarray:
    ...
    if not np.issubdtype(raw.dtype, np.number):
        raise TypeError(
            f"Momentum shard {key} must contain numeric integer grid indices: {shard['path']}"
        )
    intp_info = np.iinfo(np.dtype(np.intp))
    if np.issubdtype(raw.dtype, np.integer):
...
def _load_momentum_grid(
    ...
    event_ids = np.asarray(
        sorted({int(event_id) for shard in shards for event_id in shard["event_ids"]}),
        dtype=int,
    )
    if event_ids.size == 0:
    ...
    for shard in shards:
        ...
        for local_row, event_id in enumerate(shard["event_ids"]):
            ...
            elif n_time[row] != int(shard["n_time"][local_row]) or n_spikes[row] != int(
                shard["n_spikes"][local_row]
            ):
                raise ValueError(f"Event metadata differs for event {int(event_id)} in {shard['path']}")
            runtime_s_by_event[row] += per_event_runtime
...
def aggregate(base_dir: Path, shard_glob: str, outdir: Path) -> None:
    ...
    if base_event_ids != momentum_event_ids:
        ...
        raise ValueError(
            ...
            f"Missing in base: {missing_in_base[:20]}; "
            f"missing in momentum: {missing_in_momentum[:20]}"
        )
    ...
    event_ids = np.asarray(metadata["event_ids"], dtype=int)
    momentum_best = pd.DataFrame(
        best_grid_params("momentum", event_ids, momentum_params, momentum_grid)
    )
    ...
    random_effects = pd.DataFrame(
        random_effects_model_probabilities(pivot[models].to_numpy(float), models)
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Aggregate sharded KD momentum-grid evidence with base KD model scores."
    )
    parser.add_argument("--base-dir", required=True)
    parser.add_argument("--shard-glob", required=True)
    parser.add_argument("--output", default="results/kd-model-evidence")
