...
def _prefix_emissions(emissions: LogEmissionTensor, stop: int) -> LogEmissionTensor:
    ...
    if stop < 0 or stop > emissions.n_time:
        raise ValueError(f"prefix stop must be between 0 and {emissions.n_time}, got {stop}")
    ...
    prefix.transition_durations = transition_durations_s(emissions)[: max(stop - 1, 0)]
    return prefix
