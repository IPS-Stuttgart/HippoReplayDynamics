...
The same runtime patch also keeps paired-model threshold selection numeric after
CSV round-trips.  Pandas object/string columns otherwise sort lexicographically,
so threshold ``"10"`` can be selected before ``"2"`` and a fallback can prefer
``"10"`` false-positive claims over ``"2"``.
"""
...
_SELECTOR_WRAPPER_FLAG = "_advanced_result_margin_numeric_selector_wrapper"


def apply_advanced_result_margin_duplicate_patch() -> None:
    """Install distinct-model margins and numeric threshold selection."""

    from . import advanced_result_diagnostics as diagnostics

    current_margin = diagnostics.evidence_margin_table
    if not getattr(current_margin, _WRAPPER_FLAG, False):
    ...
    current_selector = diagnostics.select_paired_model_margin_threshold
    if not getattr(current_selector, _SELECTOR_WRAPPER_FLAG, False):
        ...
        def select_paired_model_margin_threshold(
            ...
            *,
            ...
            prepared = _numeric_threshold_selection_columns(threshold_sweep)
        ...
        diagnostics.select_paired_model_margin_threshold = (
            select_paired_model_margin_threshold
        )
...
def _numeric_threshold_selection_columns(frame: pd.DataFrame) -> pd.DataFrame:
    """Canonicalize threshold-selection metrics before comparisons and sorting."""
