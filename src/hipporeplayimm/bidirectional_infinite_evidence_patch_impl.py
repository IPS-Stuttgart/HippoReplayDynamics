"""Implementation for finite bidirectional mixtures and trajectory-return guards."""
...
_REVERSE_SCORE_FLAG = "_return_trajectory_validation_reverse_score_patch_applied"
...
def _safe_mixture_log_posterior(log_posteriors: object, weights: np.ndarray) -> np.ndarray | None:
    ...
    stacked = np.stack([current + np.log(weight) for current, weight in valid], axis=0)
...
def _normalize_return_trajectory_request(return_trajectory: object) -> bool | None:
...
def _patch_reverse_return_trajectory_validation(module: object) -> None:
    current = module.ReverseTimeReplayModel.score
    if getattr(current, _REVERSE_SCORE_FLAG, False):
        return

    @wraps(current)
    def score(self, emissions, bin_centers, *, occupancy_s=None, candidate_indices=None, return_trajectory=None):
        return_trajectory = _normalize_return_trajectory_request(return_trajectory)
        return current(
            self,
            emissions,
            bin_centers,
    ...
    setattr(score, _REVERSE_SCORE_FLAG, True)
    setattr(score, "__hipporeplayimm_original__", current)
    module.ReverseTimeReplayModel.score = score
...
def apply_bidirectional_infinite_evidence_patch() -> None:
    ...
    _patch_reverse_return_trajectory_validation(compat)
    _patch_reverse_return_trajectory_validation(direct)
    ...
    def compat_score(self, emissions, bin_centers, *, occupancy_s=None, candidate_indices=None, return_trajectory=None):
        ...
        terminal = _safe_mixture_log_posterior(
            [_terminal_log_posterior_from_score(forward), _terminal_log_posterior_from_score(reverse)], weights
        )
        trajectory = None
