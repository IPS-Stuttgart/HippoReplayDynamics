...
def _add_goal_state_space_arguments(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        ...
        help="Diffusive transition scale for exact goal-conditioned state-space models.",
    )
    parser.add_argument(
        "--goal-state-space-drift-speed-cm-s",
        type=float,
        default=DEFAULT_GOAL_DRIFT_SPEED_CM_S,
        help="Deterministic drift speed toward each fixed candidate goal.",
    )
    parser.add_argument(
        ...
        help=(
            "Sparse transition support radius, in transition sigmas, for exact "
            "goal-conditioned state-space models."
        ),
    )
