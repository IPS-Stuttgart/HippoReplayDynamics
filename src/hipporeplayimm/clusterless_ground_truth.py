...
def _compare_clusterless_scores_to_ground_truth(gt, root, scores_frame: pd.DataFrame, **kwargs) -> pd.DataFrame:
    ...
    decode_group_columns = gt._decode_group_columns(scores_frame, benchmark_decode)
    group_by = decode_group_columns[0] if len(decode_group_columns) == 1 else decode_group_columns
    for group_key, session_scores in scores_frame.groupby(group_by, sort=False):
        group_values = gt._group_key_values(decode_group_columns, group_key)
        ...
        if benchmark_decode:
            ...
            clusterless_train_session = gt._session_with_mark_cell_subset(session, train_cells, role="train")
            clusterless_joint_session = gt._session_with_mark_cell_subset(
                session,
                np.concatenate([train_cells, test_cells]),
                role="joint",
            )
            clusterless_encoding = gt.fit_clusterless_mark_encoding(
            ...
            clusterless_encoding = gt.fit_clusterless_mark_encoding(session, clusterless_config)
        for event_index, event_scores in session_scores.groupby("event_index", sort=False):
            if benchmark_decode:
                ...
                train_emissions = gt.build_clusterless_mark_emissions(
                    clusterless_train_session,
                    clusterless_encoding,
                    int(event_index),
                    emission_config,
                )
                joint_emissions = gt.build_clusterless_mark_emissions(
                ...
                emissions = gt.build_clusterless_mark_emissions(
                    session,
                    clusterless_encoding,
                    int(event_index),
                    emission_config,
