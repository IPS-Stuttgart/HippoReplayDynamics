"""Support one-bin KD momentum evidence calls."""

from __future__ import annotations

import numpy as np

_PATCHED_FLAG = "_kd_single_bin_momentum_guard_applied"


def apply_kd_single_bin_momentum_guard() -> None:
    """Patch second-order KD evidence so one-bin events use the one-bin marginal."""
    ...
    def second_order_separable_log_evidence(log_emissions, n_bins, initial, transition):
        ...
        if values.shape[0] != 1:
            return original(values, n_bins, initial, transition)
...
__all__ = ["apply_kd_single_bin_momentum_guard"]
