...
def add_model_averaged_endpoint_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for row_positions, group in _model_average_groups(out):
        ...
        # Evidence ranking must not depend on whether a model exposes a usable
        # decoded endpoint. Keep the full comparable evidence set for the
        # winner/runner-up margin, then restrict only the endpoint average.
