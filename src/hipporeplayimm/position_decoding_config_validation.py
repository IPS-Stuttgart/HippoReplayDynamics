...
def apply_position_decoding_config_validation_patch() -> None:
    ...
    if not validate_is_current:
        ...
        def validate_session_position_decoding_with_config_validation(
            session: Any,
            config: Any = None,
        ) -> Any:
        ...
        setattr(
            validate_session_position_decoding_with_config_validation,
            _VALIDATE_WRAPPER_FLAG,
            True,
        )
        validation.validate_session_position_decoding = (
            validate_session_position_decoding_with_config_validation
        )
    ...
    if not mask_is_current:
        ...
        def fit_place_field_encoding_for_position_mask_with_mask_validation(
            session: Any,
            train_frame_mask: Any,
            config: Any = None,
        ) -> Any:
            ...
            return original_fit_place_field_encoding_for_position_mask(
                session,
                mask,
                encoding_config,
            )
        ...
        setattr(
            fit_place_field_encoding_for_position_mask_with_mask_validation,
            _MASK_WRAPPER_FLAG,
            True,
        )
        validation.fit_place_field_encoding_for_position_mask = (
            fit_place_field_encoding_for_position_mask_with_mask_validation
        )
    ...
    if not counts_is_current:
        ...
        def spike_counts_for_window_with_cell_id_validation(
            session: Any,
            encoding: Any,
            start: float,
            end: float,
            ...
            return _spike_counts_for_window_by_cell_id(
                validation,
                session,
                encoding,
                start,
        ...
        setattr(
            spike_counts_for_window_with_cell_id_validation,
            _COUNTS_WRAPPER_FLAG,
            True,
        )
        validation._spike_counts_for_window = (
            spike_counts_for_window_with_cell_id_validation
        )
    ...
    if not distance_is_current:
        ...
        setattr(
            distance_with_overflow_safe_norm,
            _DISTANCE_WRAPPER_FLAG,
            True,
        )
...
def _validated_position_decoding_config(config: Any) -> Any:
    updates = {
        "decode_bin_s": _positive_finite_scalar(
            "decode_bin_s",
            getattr(config, "decode_bin_s"),
        ),
    ...
    if max_windows is not None:
        updates["max_windows_per_session"] = _positive_integer(
...
def _validated_train_frame_mask(
    train_frame_mask: Any,
    expected_length: int,
) -> np.ndarray:
    ...
    if raw.shape != (int(expected_length),):
        raise ValueError(
            "train_frame_mask must have one value per cleaned position frame"
        )
    if np.issubdtype(raw.dtype, np.bool_):
        return raw.astype(bool, copy=False)
    if _contains_textual_or_complex_mask_values(raw):
        raise ValueError(
            "train_frame_mask must contain boolean or numeric 0/1 values"
        )
    try:
    ...
    if not np.all(np.isfinite(numeric)):
        raise ValueError(
            "train_frame_mask must contain finite boolean or numeric 0/1 values"
        )
...
def _contains_textual_or_complex_mask_values(values: Any) -> bool:
    ...
    if raw.dtype == object:
        return any(
            isinstance(value, (str, bytes, complex, np.complexfloating))
            for value in raw.reshape(-1)
        )
    return False


def _validate_position_decoding_cell_ids(
    session: Any,
    encoding_config: Any,
) -> None:
    ...
    if bool(getattr(encoding_config, "use_excitatory", True)):
        _validate_optional_integral_ids(
            getattr(session, "excitatory_neurons", np.empty(0)),
            "excitatory_neurons",
        )
    _validate_optional_integral_ids(
        getattr(session, "inhibitory_neurons", np.empty(0)),
        "inhibitory_neurons",
    )
...
def _validated_encoding_cell_ids(encoding: Any) -> np.ndarray:
    cell_ids = _coerce_integral_ids(
        getattr(encoding, "cell_ids"),
        "encoding.cell_ids",
    )
    n_cells = int(getattr(encoding, "n_cells"))
    if cell_ids.shape != (n_cells,):
        raise ValueError(
            "encoding.cell_ids must contain one ID per encoding row"
        )
...
def _spike_counts_for_window_by_cell_id(
    validation: Any,
    session: Any,
    encoding: Any,
    start: float,
    ...
    spikes, _ = validation._spikes_and_cell_ids_for_encoding(
        session,
        encoding.config,
    )
    ...
    if spikes.ndim != 2 or spikes.shape[1] < 2:
        raise ValueError(
            "spikes must be two-dimensional with at least time and cell-id columns"
        )

    spike_times = np.asarray(spikes[:, 0], dtype=float)
    spike_cell_ids = _coerce_integral_ids(spikes[:, 1], "spike cell IDs")
    keep = (
        (spike_times >= float(start))
        & (spike_times < float(end))
        & np.isin(spike_cell_ids, cell_ids)
    )
    if not np.any(keep):
        return counts
...
def _positive_finite_scalar(name: str, value: Any) -> float:
    ...
    value = _reject_array_shaped_scalar(
        name,
        value,
...
def _integer_value(name: str, value: Any) -> int:
    ...
    if isinstance(item, (str, np.str_, bytes, np.bytes_)):
        try:
            ...
            raise ValueError(message) from exc
        try:
            decimal_value = Decimal(text.strip())
...
def _reject_array_shaped_scalar(
    name: str,
    value: Any,
    message: str,
) -> Any:
