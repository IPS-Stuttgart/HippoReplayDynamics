...
from aggregate_all_session_model_evidence import (
    ...
    leave_one_rat_out_exact_trajectory_nontrajectory_threshold_sensitivity,
    ...
    rat_bootstrap_exact_trajectory_nontrajectory_threshold_sensitivity,
...
def test_all_session_model_evidence_workflow_exports_expected_outputs():
    ...
    assert "exact_trajectory_nontrajectory_margin_decisions.csv" in workflow
    assert "exact_trajectory_nontrajectory_margin_summary.csv" in workflow
    ...
    assert "rat_exact_trajectory_nontrajectory_threshold_sensitivity.csv" in workflow
    assert "leave_one_rat_out_exact_trajectory_nontrajectory_threshold_sensitivity.csv" in workflow
...
def test_all_session_exact_trajectory_nontrajectory_margin_tables():
    frame = pd.DataFrame(
        [
            ...
            _paired_score_row("Rat1/Open2", 1, "sorted-spike-state-space-stationary", 5.0),
            _paired_score_row("Rat1/Open2", 1, "sorted-spike-state-space-diffusion", 3.0),
            _paired_score_row("Rat1/Open2", 1, "sorted-spike-state-space-fragmented", 2.0),
            _paired_score_row("Rat1/Open2", 1, "sorted-spike-state-space-first-order-imm", 1.0),
            _paired_score_row(
                "Rat1/Open2",
                1,
                "sorted-spike-state-space-momentum-exact-sparse",
            ...
            _paired_score_row("Rat2/Open1", 2, "sorted-spike-state-space-stationary", 7.0),
            _paired_score_row("Rat2/Open1", 2, "sorted-spike-state-space-diffusion", 2.0),
            _paired_score_row("Rat2/Open1", 2, "sorted-spike-state-space-fragmented", 1.0),
            _paired_score_row("Rat2/Open1", 2, "sorted-spike-state-space-first-order-imm", 0.0),
            _paired_score_row(
                "Rat2/Open1",
                2,
                "sorted-spike-state-space-momentum-exact-sparse",
    ...
    decisions = exact_trajectory_nontrajectory_margin_decisions(frame, margin_threshold=2.0)

    assert decisions["margin_decision"].tolist() == ["trajectory", "ambiguous", "nontrajectory"]
    assert decisions["best_nontrajectory_model"].tolist() == ["sorted-spike-state-space-stationary"] * 3
    assert decisions["trajectory_minus_nontrajectory_log_evidence"].tolist() == [8.0, -1.0, -5.0]

    summary = exact_trajectory_nontrajectory_margin_summary(decisions).iloc[0]
    assert summary["events"] == 3
    assert summary["required_complete_events"] == 3
    assert summary["trajectory_raw_wins"] == 1
    assert summary["nontrajectory_raw_wins"] == 2
    assert summary["trajectory_confident_claims"] == 1
    assert summary["nontrajectory_confident_claims"] == 1
    assert summary["ambiguous_events"] == 1
    assert summary["trajectory_confident_claim_fraction"] == pytest.approx(1 / 3)
    assert summary["mean_trajectory_minus_nontrajectory_log_evidence"] == pytest.approx(2 / 3)

    threshold_summary = exact_trajectory_nontrajectory_threshold_sensitivity(
        frame,
        thresholds=(0.0, 2.0),
    )
    assert threshold_summary["trajectory_confident_claims"].tolist() == [1, 1]
    assert threshold_summary["nontrajectory_confident_claims"].tolist() == [2, 1]

    session_summary = session_exact_trajectory_nontrajectory_threshold_sensitivity(
        frame,
        thresholds=(2.0,),
    )
    assert session_summary["session"].tolist() == ["Rat1/Open1", "Rat1/Open2", "Rat2/Open1"]
    assert session_summary["trajectory_confident_claims"].tolist() == [1, 0, 0]

    rat_summary = rat_exact_trajectory_nontrajectory_threshold_sensitivity(
        frame,
        thresholds=(2.0,),
    )
    assert rat_summary["rat"].tolist() == ["Rat1", "Rat2"]
    assert rat_summary["trajectory_confident_claims"].tolist() == [1, 0]
    assert rat_summary["nontrajectory_confident_claims"].tolist() == [0, 1]

    leave_one = leave_one_rat_out_exact_trajectory_nontrajectory_threshold_sensitivity(
        frame,
        thresholds=(2.0,),
    )
    assert leave_one["held_out_rat"].tolist() == ["Rat1", "Rat2"]
    assert leave_one["trajectory_confident_claims"].tolist() == [0, 1]
    assert leave_one["nontrajectory_confident_claims"].tolist() == [1, 0]

    bootstrap = rat_bootstrap_exact_trajectory_nontrajectory_threshold_sensitivity(
        frame,
        thresholds=(2.0,),
        n_bootstrap=50,
        random_seed=7,
    ).iloc[0]
    assert bootstrap["observed_events"] == 3
    assert bootstrap["observed_rats"] == 2
...
def test_all_session_exact_trajectory_nontrajectory_gate_summary_reports_family_readiness():
    ...
    assert bool(summary.loc["all_rats_family_median_delta_positive", "passed"])
    assert bool(summary.loc["leave_one_rat_out_family_claim_majority", "passed"])
    assert bool(summary.loc["leave_one_rat_out_no_nontrajectory_claims", "passed"])
    assert bool(summary.loc["leave_one_rat_out_family_mean_delta_positive", "passed"])
    assert bool(summary.loc["leave_one_rat_out_family_median_delta_positive", "passed"])
...
def test_all_session_exact_trajectory_nontrajectory_gate_summary_rejects_single_rat_dominance():
    rows: list[dict[str, object]] = []
    for event_index in range(30):
        rows.extend(
            [
                _paired_score_row("Rat1/Open1", event_index, "sorted-spike-state-space-stationary", -3.0),
                _paired_score_row("Rat1/Open1", event_index, "sorted-spike-state-space-diffusion", 0.0),
                _paired_score_row("Rat1/Open1", event_index, "sorted-spike-state-space-fragmented", -2.0),
                _paired_score_row("Rat1/Open1", event_index, "sorted-spike-state-space-first-order-imm", -1.0),
                _paired_score_row(
                    "Rat1/Open1",
                    event_index,
                    "sorted-spike-state-space-momentum-exact-sparse",
    ...
    for rat in range(2, 5):
        for event_index in range(3):
            rows.extend(
                [
                    _paired_score_row(f"Rat{rat}/Open1", event_index, "sorted-spike-state-space-stationary", 0.0),
                    _paired_score_row(f"Rat{rat}/Open1", event_index, "sorted-spike-state-space-diffusion", 1.0),
                    _paired_score_row(f"Rat{rat}/Open1", event_index, "sorted-spike-state-space-fragmented", 2.0),
                    _paired_score_row(
                        f"Rat{rat}/Open1",
                        event_index,
                        "sorted-spike-state-space-first-order-imm",
    ...
    assert bool(summary.loc["trajectory_family_confident_claim_majority", "passed"])
    assert not bool(summary.loc["leave_one_rat_out_family_claim_majority", "passed"])
    assert not bool(summary.loc["overall", "passed"])


def test_all_session_exact_trajectory_nontrajectory_gate_summary_rejects_weak_individual_rat():
    rows: list[dict[str, object]] = []
    for session_idx in range(1, 3):
        session = f"Rat1/Open{session_idx}"
        for event_index in range(5):
            trajectory_log_evidence = 8.0 if event_index == 0 else 2.0
            rows.extend(
                [
