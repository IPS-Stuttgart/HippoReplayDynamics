...
def test_compare_ground_truth_groups_in_memory_cell_id_values(monkeypatch) -> None:
    scores = pd.DataFrame(
        {
            "session": ["s1", "s1"],
            "event_index": [0, 0],
            ...
            "benchmark_test_cell_fraction": [0.5, 0.5],
            "benchmark_random_seed": [11, 11],
            "train_cell_ids": [[1], np.asarray([2], dtype=int)],
            "test_cell_ids": [np.asarray([2], dtype=int), [1]],
        }
    )
    labels = pd.DataFrame(
        {
    ...
    class FakeEncoding:
        ...
        def select_cells(self, cell_ids):
            selected_cells.append(tuple(np.asarray(cell_ids, dtype=int)))
            return self
    ...
    monkeypatch.setattr(ground_truth, "_build_models", lambda *args, **kwargs: {"random": FakeModel()})

    comparison = ground_truth.compare_scores_to_ground_truth("unused-root", scores)

    assert selected_cells == [(1,), (1, 2), (2,), (1, 2)]
