...
def test_compare_ground_truth_scopes_cell_split_metadata_within_session_without_split_index(monkeypatch) -> None:
    scores = pd.DataFrame(
        {
            "session": ["s1", "s1"],
            "event_index": [0, 1],
            ...
            "train_log_likelihood": [0.0, 0.0],
            "joint_log_likelihood": [0.0, 0.0],
            "benchmark_test_cell_fraction": [0.5, 0.25],
            "benchmark_random_seed": [11, 19],
            "benchmark_cell_split_seed": [13, 23],
            "benchmark_cell_split_strategy": ["random", "peak-rate"],
            "benchmark_cell_split_strata": [4, 6],
        }
    )
    ...
    captured_splits: list[tuple[str, int, float, int]] = []
    ...
    assert set(comparison["event_index"]) == {0, 1}
