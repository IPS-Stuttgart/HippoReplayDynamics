...
def test_build_models_includes_anchored_trajectory_imm_variant():
    models = _build_models(
        BenchmarkConfig(
            models=("sorted-spike-state-space-trajectory-imm-anchored-exact-sparse",)
        )
    )

    model = models["sorted-spike-state-space-trajectory-imm-anchored-exact-sparse"]
    assert model.name == "sorted-spike-state-space-trajectory-imm-anchored-exact-sparse"
    assert model.mode == "trajectory-imm-exact-sparse"
    assert model.config.trajectory_imm_momentum_initial_probability == 0.05
...
def test_add_relative_metrics_uses_only_comparable_state_space_single_mode_baselines():
    rows = pd.DataFrame(
        {
            "session": ["s1", "s1", "s1", "s1"],
            "event_index": [7, 7, 7, 7],
    ...
    assert deltas["sorted-spike-state-space-diffusion"] == 0.0
    assert np.isnan(deltas["sorted-spike-state-space-momentum"])
    assert deltas["sorted-spike-state-space-jump"] == -1.0
    assert np.isnan(deltas["sorted-spike-state-space-imm"])
    assert result["best_static_heldout_log_likelihood"].eq(-8.0).all()
...
def test_add_relative_metrics_uses_only_comparable_clusterless_single_mode_baselines():
    rows = pd.DataFrame(
        {
            "session": ["s1", "s1", "s1", "s1"],
            "event_index": [7, 7, 7, 7],
    ...
    assert deltas["clusterless-state-space-diffusion"] == 0.0
    assert np.isnan(deltas["clusterless-state-space-momentum"])
    assert deltas["clusterless-state-space-jump"] == -1.0
    assert np.isnan(deltas["clusterless-state-space-imm"])
    assert result["best_static_heldout_log_likelihood"].eq(-8.0).all()
...
def test_add_relative_metrics_does_not_treat_restricted_heldout_as_lower_bound():
    rows = pd.DataFrame(
        {
    ...
    for model in ("diffusion", "momentum", "imm"):
        assert by_model.loc[model, "evidence_support"] == RESTRICTED_HELDOUT_EVIDENCE_SUPPORT
        assert np.isnan(by_model.loc[model, "delta_vs_best_static"])
        assert np.isnan(by_model.loc[model, "lower_bound_delta_vs_best_static"])
        assert np.isnan(by_model.loc[model, "delta_vs_best_static_truncated_lower_bound"])
    assert by_model["best_static_truncated_lower_bound_heldout_log_likelihood"].isna().all()
...
def test_add_relative_metrics_parses_string_false_comparability():
    ...
    assert result.loc["diffusion", "evidence_support"] == RESTRICTED_HELDOUT_EVIDENCE_SUPPORT
    assert np.isnan(result.loc["diffusion", "lower_bound_delta_vs_best_static"])


def test_benchmark_summary_separates_exact_and_restricted_heldout_support():
    rows = pd.DataFrame(
        {
            "session": ["s1", "s1", "s1"],
            "event_index": [7, 7, 7],
    ...
    assert summary.loc["diffusion", "evidence_support"] == RESTRICTED_HELDOUT_EVIDENCE_SUPPORT
    assert np.isnan(summary.loc["diffusion", "mean_delta_vs_best_static"])
    assert np.isnan(summary.loc["diffusion", "mean_lower_bound_delta_vs_best_static"])
