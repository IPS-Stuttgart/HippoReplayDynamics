...
def test_cell_split_scope_grouping_distinguishes_scalar_strings_from_sequences() -> None:
    scores = pd.DataFrame(
        {
            "session": ["Rat1/Open1", "Rat1/Open1"],
            "event_index": [0, 0],
            ...
            "benchmark_cell_split_strategy": ["explicit", "explicit"],
            "benchmark_cell_split_strata": [2, 2],
            "train_cell_ids": [[1, 2], "[1, 2]"],
            "test_cell_ids": [[3], "[3]"],
        }
    )
    ...
    assert len(groups) == 2
    assert isinstance(groups[0]["train_cell_ids"].iloc[0], list)
    assert groups[1]["train_cell_ids"].iloc[0] == "[1, 2]"
