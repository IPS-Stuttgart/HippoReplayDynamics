...
def test_clusterless_benchmark_scores_train_and_joint_with_train_fitted_encoder(monkeypatch):
    ...
    def fake_score_train_joint_model(model, train_emissions, joint_emissions, bin_centers):
        assert bin_centers is shared_encoder.bin_centers
        return (
    ...
    train_cell_set = emission_calls[0][0]
    assert train_cell_set in {(1,), (2,)}
    assert fitted_cell_sets == [train_cell_set]
    assert emission_calls[1][0] == (1, 2)
    assert emission_calls[0][1] is shared_encoder
    assert emission_calls[1][1] is shared_encoder
