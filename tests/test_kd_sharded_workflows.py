...
from plan_kd_event_shards import _event_chunks, _event_spec, _validate_max_events  # noqa: E402
...
def test_max_events_validation_rejects_non_integer_values():
    assert _validate_max_events(None) is None
    assert _validate_max_events(np.int64(3)) == 3

    with pytest.raises(ValueError, match="integer"):
        _validate_max_events(2.5)  # type: ignore[arg-type]
    with pytest.raises(ValueError, match="integer"):
        _validate_max_events(True)  # type: ignore[arg-type]
    with pytest.raises(ValueError, match="non-negative"):
        _validate_max_events(-1)
