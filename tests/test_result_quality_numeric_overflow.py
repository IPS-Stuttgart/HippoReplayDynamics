...
def test_margin_annotation_ignores_boolean_log_evidence_cells() -> None:
    scores = pd.DataFrame(
        [
            {
                "status": "success",
    ...
    scores["log_evidence"] = pd.Series([True, 0.5, 0.25], dtype=object)

    annotated = add_evidence_margin_columns(scores)

    assert annotated["exact_model_best_model"].tolist() == ["best"] * 3
    assert annotated["exact_model_log_evidence_margin"].tolist() == [0.25] * 3
    np.testing.assert_array_equal(
        annotated["exact_model_rank"].to_numpy(),
