...
def test_momentum_candidates_reject_zero_transition_sigma_series() -> None:
    emissions, bin_centers, candidates = _candidate_inputs()

    with pytest.raises(ValueError, match="transition_sigmas_cm values must be > 0"):
        _score_momentum_candidates(
            ...
            velocity_decay=0.95,
            transition_sigmas_cm=np.array([1.0, 0.0], dtype=float),
        )


def test_momentum_candidates_reject_negative_velocity_decay_series() -> None:
    emissions, bin_centers, candidates = _candidate_inputs()

    with pytest.raises(ValueError, match="velocity_decays values must be >= 0"):
        _score_momentum_candidates(
            emissions,
            bin_centers,
            candidates,
            ...
            initial_sigma_cm=1.0,
            velocity_decay=0.95,
            velocity_decays=np.array([0.95, -0.1], dtype=float),
        )


def test_imm_candidates_reject_zero_diffusion_sigma_series() -> None:
    emissions, bin_centers, candidates = _candidate_inputs()

    with pytest.raises(ValueError, match="diffusion_sigmas_cm values must be > 0"):
        _score_imm_candidates(
            ...
            diffusion_sigmas_cm=np.array([1.0, 0.0], dtype=float),
        )
