...
def test_runtime_patches_restore_trajectory_imm_validation_after_reload() -> None:
    ...
    # importlib.reload() keeps names that the new module body does not redefine,
    # so the old module-level patch flag survives while the wrapped functions do
    # not. This is the stale state that previously made runtime refresh a no-op.
