...
def test_exact_model_transition_duration_helpers_reject_corrupt_values(coerce_durations) -> None:
    with pytest.raises(ValueError, match="transition durations"):
        coerce_durations([0.01, np.nan], n_time=3, fallback_dt=0.01)
    with pytest.raises(ValueError, match="transition durations"):
        coerce_durations([0.01, 0.0], n_time=3, fallback_dt=0.01)
    with pytest.raises(ValueError, match="transition durations"):
        coerce_durations([0.01, -0.02], n_time=3, fallback_dt=0.01)

    # Legacy direct callers that omit duration metadata still get uniform-dt
    # fallback, but the fallback itself must be valid.
...
def test_exact_model_duration_decay_helpers_reject_corrupt_durations(duration_decays) -> None:
